...
//Did some bit of guess work will have to revisit
}
function createTimeEvent(employee, date, type){
    const dateworked = /^\d{4}-\d{2}-\d{2} \d{4}$/
    if (date.match(dateworked) === null) {
    ...
    type === "TimeIn" ? employee.timeInEvents.push(entry):employee.timeOutEvents.push(entry)
    return employee
...
//I have done guess work and will have to go through this again
